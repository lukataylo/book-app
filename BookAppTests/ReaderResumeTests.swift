...
struct ReaderResumeTests {
    ...
    @Test
    func paragraphLocatorRestoresExactPosition() async throws {
        let container = try makeContainer()
        let context = container.mainContext

        let book = Book(title: "Test", author: "Author", format: .epub)
        let body = (0..<10).map { paragraph($0) }.joined(separator: "\n\n")
        let variant = BookVariant(book: book, kind: .original)
        context.insert(book)
        context.insert(variant)
        variant.writeText(body)
    ...
    @Test
    func legacyScrollLocatorFallsBackToPercentEstimate() async throws {
        let container = try makeContainer()
        let context = container.mainContext
    ...
    @Test
    func noProgressMeansFreshOpenAtTop() async throws {
        let container = try makeContainer()
        let context = container.mainContext
    ...
    @Test
    func outOfRangeLocatorClamps() async throws {
        let container = try makeContainer()
        let context = container.mainContext
